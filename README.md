...
int operation(int& N, int* A)
{
	...
	for (int i = 0; i < N; i++)  scanf_s("%d", &A[i]); //Заполняем массив
	if (A[0] % 2 != 0) flag = true; else flag = false;
	for (int i = 0; i < N; i++)
	{
		if (!(A[i] % 2) && !flag) flag = true; //Если A[i] чётное и флаг был нечётным, то флагменяем на чётное
		else if (A[i] % 2 && flag) flag = false; //Если A[i] нечётное и флаг был чётным, тофлаг меняем на нечётное
		else //Иной случай говорит нам о том, что чередование нарушено
		{
			...
			break; //Выходим из цикла, так как мы нашли элемент, нарушающийчередование
		}
	}
...
{
	for (int k = i - word; k < i - 1; k++) //Прогоняем слово начиная от первой буквы изаканчивая предпоследней
...
{
if ((s[i] == '\0') && (word == 0)) //Если встретили нуль-терминатор и проделали всеоперации(word = 0), то выходим из цикла
break;
else if ((s[i] != ' ') && (s[i] != '\0')) //Считаем количество букв в слове
word++;
else
{
	if (unique(s, i, word)) count += 1; //Проверяем, не повторяются ли в слове буквы,если нет, то добавляем к счётчику единицу
...
void operation(char* s)
{
	for (int i = 0; i <= strlen(s); i++) //Пробегаем через всю строку (включаянуль - терминатор)
	{
		...
		{
			for (int k = i - 1; k < strlen(s); k++) s[k] = s[k + 1]; //То сдвигаем массив влево наодин элемент
...
{
	if (B < A) A = A + B - (B = A); //Если B < A, то меняем их мистами, путём нехитройзамены
	int s = 0;
	for (int i = A + 1; i < B; i++) { s += i; } //Находим сумму элементов между A и B
	for (int i = 0; i < s; i++) { printf("%d", s, ' '); }//Выводим сумму
}
int main()
{
